-- Runs umeed-poll-due-work every minute (Implementation.md §11.5's "short
-- interval" guidance for the scheduler this replaces server-side). The
-- bearer token sent here is a custom invoke secret (UMEED_POLL_DUE_WORK_INVOKE_SECRET
...
select cron.schedule(
  ...
  select net.http_post(
    url := current_setting('app.settings.supabase_url') || '/functions/v1/umeed-poll-due-work',
