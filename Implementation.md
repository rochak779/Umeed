...
- **Cron migration `20260904133314_fix_poll_due_work_cron_secret.sql` hardcodes this dev project's URL.** Harmless today (single project, URL isn't secret), but applying this migration to a different project (e.g. a future staging/production Supabase project) would schedule a job that fires against *this* dev project's Edge Function using the new project's Vault secret — a silent cross-environment write path. Move the URL into Vault alongside the secret, or a settings table, before provisioning a second environment.
