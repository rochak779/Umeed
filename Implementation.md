...
### Phase 10 known gaps (found during implementation and a final whole-branch review; not yet closed)

- **Client-side (browser) code cannot actually reach Supabase in `DATA_ADAPTER=supabase` mode yet — this is the most important gap, correcting an earlier draft of this note that called the browser path "verified working."** Vite only injects `import.meta.env.VITE_*`-prefixed variables into the client bundle (confirmed by grepping the built output); a bare `process.env["DATA_ADAPTER"]` — which `container.ts` reads for both `buildAuthProvider()` and `buildRepositories()` — is always `undefined` in the browser. Practical effect: server-side rendering correctly uses `SupabaseAuthProvider` and real Supabase repositories, but the moment the page hydrates in the browser, both silently fall back to `LocalAuthProvider` and local (in-memory/localStorage) repositories, regardless of the configured adapter. `useAlertsRealtime`'s guard has the same problem, so the Realtime hook never actually subscribes client-side either. **This is not a small fix**: `buildRepositories()` currently constructs every Supabase repository with the **service-role** client, which can never be safely exposed to the browser (that would bypass Row Level Security entirely for anyone with devtools open). A real fix means giving client-side repositories an anon-key client authenticated with the signed-in user's own session — respecting the RLS policies already defined in the Phase 9 migrations — not simply adding `VITE_`-prefixed env vars. This needs its own design pass before Phase 10's Task 11 (flipping local dev to `DATA_ADAPTER=supabase`) can actually work end-to-end.
- **The `poll-due-work` cron job is deployed and was verified working end-to-end via direct database evidence** (a `cron.job_run_details` row shows `status: succeeded`) **but is deliberately left unscheduled (paused).** At verification time it immediately began generating alerts against leftover dev/test-fixture care circles (no real users exist yet). Before relying on this in production: purge stale dev fixture data from the live project, then run `select cron.alter_job(jobid, active := true) from cron.job where jobname = 'umeed-poll-due-work';` to turn it back on. (Since the move to the shared `Rochak's Hobby` project the function and job are named `umeed-poll-due-work`; the job was re-created paused there.)
- **The `alerts` table has not been added to the `supabase_realtime` publication** (no migration does this). Until it is, `useAlertsRealtime`'s `postgres_changes` subscription cannot fire even once the client-side gap above is fixed — needs `alter publication supabase_realtime add table alerts` as a migration, not a one-off dashboard click.
- **Pre-existing test-hygiene gap in `test/contracts/repositoryContract.ts`'s Supabase-backed contract suites**, unrelated to Phase 10's own work: several contract tests (`RoutineRepository`, `OccurrenceRepository`, `AlertRepository`, `AuditRepository`) use fixed fake IDs (e.g. `"r-1"`, `"routine-1"`, `"a-1"`) and rely on each test starting from an empty table — true for the in-memory local adapter, never true for the shared live Supabase database. Re-running these suites against the live project (without a full database reset in between) causes collisions and, since some of that fixture data is `enabled`/`active` by design, can also cause the scheduler to backfill months of occurrences for it. Needs proper per-test teardown (or randomized IDs, matching the pattern the auth/RLS contract tests already use) before this suite can be trusted as a repeatable CI gate. `SupabaseAuthProvider.contract.test.ts` has the same shape of gap for `auth.users` rows (randomized emails, so no collisions, but no teardown — accumulates real user rows on the live project on every run).
- **`test/infrastructure/supabase/pollDueWorkFunction.contract.test.ts` is currently unreliable**, as a direct consequence of the fixture-hygiene gap above: a live invocation, after the fixture history it depends on has been deleted/reset, must regenerate months of backfilled occurrences via sequential per-row inserts inside a single Edge Function call, which can exceed even a 30s test timeout. Not a defect in the Edge Function itself — its correctness was independently verified via direct `cron.job_run_details` evidence.
- **Cron migration `20260904133314_fix_poll_due_work_cron_secret.sql` hardcodes the shared `Rochak's Hobby` project's URL** (`aqvpdfdfsterpdumjglx`). Harmless today (one environment, URL isn't secret), but applying it to a different project would schedule a job that calls *this* project's Edge Function — a silent cross-environment write path. Move the URL into Vault alongside the secret before provisioning a second environment.
- **Local `.env.local` not yet flipped to `DATA_ADAPTER=supabase` for day-to-day dev**, and the README's "Running against Supabase" section is not yet written — blocked on the client-side gap above, not just an unwritten doc.
